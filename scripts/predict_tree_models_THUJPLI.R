...
folder_path <- "outputs/ranger/RFregression_classes/THUJPLI"  
...
clim2<- c("DD5", "DD_delayed", "PPT_MJ", "PPT_JAS", 
        "CMD.total", "CMI", "CMDMax", "SHM", "AHM", "NFFD", "PAS", "Species")
clim.bc<-dplyr::select(clim.bc, clim2)  
...
#make predictions
predsCwC4<-predict(object = RF_THUJPLI_C34,data =clim.bc, type='response')
...
#make predictions
predsCwA2<-predict(object = RF_THUJPLI_AB12,data =clim.bc, type='response')
...
#make predictions
predsCwE6<-predict(object = RF_THUJPLI_DE56,data =clim.bc, type='response')
...
predsCwA2.df<-process_predictions(predsCwA2)
predsCwE6.df<-process_predictions(predsCwE6)

##add lat, long back in
load(file="data/clim.bc2k.RData") #2km
predsCwA2.df<-left_join(predsCwA2.df, dplyr::select(clim.bc, id, lat, lon))%>%dplyr::select(lon, lat, ypred)%>%rename(x=lon, y=lat)
...
predsCwC4_tidy <- as_tibble(predsCwC4.r, xy = TRUE)%>%rbind
predsCwA2_tidy <- as_tibble(predsCwA2.r, xy = TRUE)%>%rbind(., dummy)
predsCwE6_tidy <- as_tibble(predsCwE6.r, xy = TRUE)%>%rbind
...
c4<-ggplot() +
  ...
  theme_classic() + theme(legend.position='none', axis.line=element_blank(),axis.text=element_blank(), axis.ticks = element_blank())
a2<-ggplot() +
  geom_raster(data = predsCwA2_tidy, aes(x = x, y = y, fill = factor(round(ypred), levels=0:5))) +
  scale_fill_grass_d(palette = 'water', direction = -1, breaks=0:5,
                     name = "Abundance class", labels = c("absent", "<1%", "1-5%", "5-25%", "25-50%", ">50%")) +
  labs(title = "Cw A2") +   xlab(" ") + ylab(" ")+ 
  #geom_sf(data = bcboundlow.reproj, fill = NA, color='grey')+ 
  theme_classic() + theme(legend.position='none', axis.line=element_blank(),axis.text=element_blank(), axis.ticks = element_blank())
e6<-ggplot() + 
...
pdf(file = "outputs/ranger/RFregression_classes/THUJPLI/THUJPLI_preds_A2C4E6.pdf", width = 12, height = 8)
ggpubr::ggarrange(plots, c4,l1, a2, e6, l2)
dev.off()

#CW seems to be under predicting the high abundance areas. 
...
ggplot() +
  ...
  #geom_sf(data = bcboundlow.reproj, fill = NA, color='grey')+ 
  theme_classic() + theme(axis.line=element_blank(),axis.text=element_blank(), axis.ticks = element_blank())+
  geom_point(data = subset(Cw,TotalA_class>4), aes(x = Longitude, y = Latitude), color= 'gold', shape=21, size=3) 
