...
2. clean_tree_data.R- Compile/filter BEC plot data for tree species (output= tree_data_cleaned.Rdata)
3. update_plot_data.R<- Update BEC plot data to newest BEC and assign missing site series calls when possible
4. suit_tables.R- pull in spp suitabiliy tables and merge with BEC data (output= feas_abundance_data.Rdata) 
5. climPCAs.R- runs PCA on plot level climate parameters and merge back with suit/abundance data- sourced in suit.tables.R (output=feas_abund_clim_data.Rdata)
6. explore_tree_models_bayesian.R- Models BEC plot data using expert informed suitability ratings as Bayesian priors
