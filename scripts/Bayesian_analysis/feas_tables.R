...
feas.dat<-subset(feas.dat, !is.na(newsuit))
...
tree_dat_sub<-dplyr::select(tree_dat, PlotNumber, Species,TotalA, TotalB, SiteUnit, Site, bgc,
                            UserSiteUnit, BECSiteUnit, GIS_BGC, 
                            SiteUnit, Site, bgc, SubZone,SiteSeries,        
...
#deal with all others that not on ss_cleaned 
tree_dat_sub<-unite(tree_dat_sub, col = "ss_nospace2", c("GIS_BGC", "SiteSeries"), sep = "/", remove=F)
tree_dat_sub<-mutate(tree_dat_sub, ss_nospace=if_else(is.na(ss_nospace), ss_nospace2, ss_nospace))
tree_dat_sub$ss_nospace2<-NULL
gc()

#updated ss from LMH77 crosswalk 
cw<-read.csv("data/LMH77_BGC_su_Crosswalk_v2.csv")
cw<-unite(cw, col = "ss_nospace", c("OldBGC","OldSiteUnitLMH28"), sep = "/", remove=F)
cw<-unite(cw, col = "ss_nospace_new", c("NewBGC","NewSiteUnit"), sep = "/", remove=F)
cw<-select(cw, ss_nospace, ss_nospace_new)%>%distinct(.)

tree_dat_sub<-left_join(tree_dat_sub, cw)
tree_dat_sub<-mutate(tree_dat_sub,ss_nospace=if_else(!is.na(ss_nospace_new), ss_nospace_new, ss_nospace))
tree_dat_sub$bgc<-NULL
...
#BEC_missing_feas_unique<-filter(BEC_missing_feas_unique, !grepl('support', ss_nospace))#what are support plots for?
sort(unique(BEC_missing_feas_unique$ss_nospace))
...
knitr::kable(group_by(feas.dat.suby, newsuit_ord)%>%summarise(counts=n())) #30 E5s with non-zero abundances 

#save feas plus plot data
feas.dat.sub<-select(feas.dat.suby, -X, -mod, -outrange, -Site, -UserSiteUnit,        
                     -BECSiteUnit, -GIS_BGC,  -SubZone, -SiteSeries, -MapUnit,             
                     -SitePlotQuality)   
...
avgs<-group_by(feas.dat.sub, zone, bgc, ss_nospace, Species, spp, newsuit_ord)%>%summarise(mean_abund_ss=mean(TotalA, na.rm = T), sd_abund_ss=sd(TotalA, na.rm = T))%>%ungroup(.)%>%
...
check<-group_by(feas.dat.sub, spp)%>%summarise(n_ratings= n_distinct(newsuit), abund=mean(TotalA, na.rm=T))
...
#Yc - good, some high 3s, 4s in CWH
...
check<-subset(avgs, newsuit_ord=="5" & mean_abund_ss>0) #few- but one Fd value very high  
