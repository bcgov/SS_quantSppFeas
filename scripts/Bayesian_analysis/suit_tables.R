...
feas.dat<-read.csv("data/Suitability_v13_30.csv") #most recent April 2026
...
feas.dat<-filter(feas.dat, !grepl('_OC|_WC|_CA|_OR|_WA|_ID|_MT|_CA|_WY|_CO|_NV|UT|BSJP|abE|abN|abS|abC|	MGPmg|FGff|
...
avgs<-group_by(feas.dat.sub, bgc, zone,ss_nospace, Species, spp, newsuit_ord)%>%
...
ggplot(subset(avgs, spp=="Ac"), aes(x = newsuit_ord, y = mean_abund_ss, alpha=0.5))+
  ...
  facet_wrap( ~ zone, scales="free_y") + theme_bw() + theme(legend.position='none') +ggtitle("At")
#At- 
...
ggplot(subset(avgs, spp=="Cw"), aes(x = newsuit_ord, y = mean_abund_ss, alpha=0.5))+
  ...
  geom_hline(aes(yintercept = 25), lty=2)
#Ep-
...
#USE plot random effect instead 
